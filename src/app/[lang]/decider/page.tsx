...
export function generateMetadata(): Metadata {
  return {
    ...
    // One English page served under every locale prefix; index only /en.
    alternates: { canonical: "/en/decider" },
    openGraph: {
      type: "website",
      title: TITLE,
      description: DESCRIPTION,
      url: "https://sponom.dev/en/decider",
      siteName: "sponom.dev",
      ...
      <footer className="relative z-1 max-w-6xl mx-auto px-6 py-12 sm:py-14 pb-28 sm:pb-14 border-t border-dc-hairline text-sm text-dc-label-2">
        <div className="grid grid-cols-2 gap-10 sm:grid-cols-[1.4fr_1fr_1fr]">
          ...
          <nav className="flex flex-col gap-3">
            ...
            <Link
              href={`/${locale}/decider/privacy/`}
              className="hover:text-dc-label transition-colors"
            >
              Privacy
            </Link>
            <Link
              href={`/${locale}/decider/terms/`}
              className="hover:text-dc-label transition-colors"
            >
              Terms
            </Link>
